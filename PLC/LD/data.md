...
- 동작 방식 : 특정 범위 $T_0\sim T_1$에 속하는 데이터를 $T_2\sim T_3$의 범위(스케일)로 바꾸고 싶을 때
- S1에 저장되어 있을 때 S2에 $T_1$의 값을 S2의 다음 디바이스에 $T_0$  값을 넣고 S3에 바꾸고 싶은 범위의 상한값이 $T_3$를, 그 다음 디바이스에 $T_4$의 값을 넣어 D에 변환된 값을 전송한다