...
lamp0 := is_running AND ton0.ET < T#1S;
lamp1 := is_running AND (ton0.ET < T#1S AND counter = 2) OR (counter >= 4);
lamp2 := is_running AND ton0.ET < T#1S AND counter < 5;
```
![[Pasted image 20260819104530.png]]
...
r_trig_3(CLK:=btn3);
f_trig_3(CLK:=btn3);
ton0(IN:=mode=1 AND NOT ton0.Q, PT:=T#4S);

IF f_trig_0.Q AND mode = 0 THEN
	mode := 1;
ELSIF f_trig_1.Q AND mode = 1 THEN
	mode := 2;
ELSIF r_trig_2.Q AND mode = 2 THEN
	mode := 0;
END_IF;
...
![[Pasted image 20260819105430.png]]
...
r_trig_3(CLK:=btn3);
f_trig_3(CLK:=btn3);
ton0(IN:=is_running AND NOT ton0.Q, PT:=T#11S);
...
lamp0 := (ton0.ET > T#2S AND ton0.ET <= T#5S) 
		OR (ton0.ET > T#9S AND ton0.ET <= T#10S);
lamp1 := (ton0.ET > T#4S AND ton0.ET <= T#7S) 
		OR (ton0.ET > T#8S AND ton0.ET <= T#9S)
		OR (ton0.ET > T#10S AND ton0.ET <= T#11S);
```
...
r_trig_3(CLK:=btn3);
f_trig_3(CLK:=btn3);
ton0(IN:=mode=3, PT:=T#99S);
ton1(IN:=mode=4 AND NOT ton2.Q, PT:=T#1S);
ton2(IN:=mode=4 AND ton1.Q, PT:=t);
ton3(IN:=mode=2 AND NOT ton3.Q, PT:=T#2S);
r_trig_4(CLK:=lamp1);
f_trig_4(CLK:=lamp1);
...
ELSIF mode = 0 AND counter0 >= 2 THEN
	mode := 1;
	//counter0 := 0;
...
ELSIF mode = 1 AND counter1 >= 2 THEN
	mode := 2;
	//counter1 := 0;
...
ELSIF f_trig_2.Q AND mode = 3 THEN
	t := ton0.ET;
	mode := 4;
	
ELSIF counter = 3 AND mode = 4 THEN
	mode := 0;
	counter := 0;
	counter0 := 0;
	counter1 := 0;	
