...
cv2.waitKey(time) # cv2.imshow()는 그려야 할 작업을 큐에 쌓아둠. cv2.waitkey가 호출되면 큐에서 값을 꺼내서 이미지를 렌더링함

alignMTB = cv2.createAlignMTB() # Median Threshold Bitmap알고리즘을 수행할 객체 생성
alignMTB.process(images, images) # 노출이 다른 여러 장의 이미지 리스트를 방아 정렬시킨 후 images 리스트에 덮어 씌움
```
